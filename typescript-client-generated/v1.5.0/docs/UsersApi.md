...
async function example() {
  ...
  const body = {
    // string | Challenge string from first request (step 2 only). Must match exactly. (optional)
    challenge: challenge_example,
  } satisfies DeleteUserAccountRequest;

  try {
    const data = await api.deleteUserAccount(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
|------------- | ------------- | ------------- | -------------|
| **challenge** | `string` | Challenge string from first request (step 2 only). Must match exactly. | [Optional] [Defaults to `undefined`] |
...
async function example() {
  ...
  try {
    const data = await api.logoutCurrentUser();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
> TransferOwnershipResult transferCurrentUserOwnership(transferOwnershipRequest)
...
async function example() {
  ...
  } satisfies TransferCurrentUserOwnershipRequest;

  try {
    const data = await api.transferCurrentUserOwnership(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
