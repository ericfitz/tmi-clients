...
> Repository patchThreatModelRepository(threatModelId, repositoryId, jsonPatchDocumentInner)
...
import type { PatchThreatModelRepositoryRequest } from '@tmiclient/client';
...
async function example() {
  ...
  const body = {
    ...
    // Array<JsonPatchDocumentInner> | JSON Patch operations to apply to the repository
    jsonPatchDocumentInner: [{"op":"replace","path":"/name","value":"Updated Name"},{"op":"add","path":"/description","value":"New description"}],
  } satisfies PatchThreatModelRepositoryRequest;

  try {
    const data = await api.patchThreatModelRepository(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **jsonPatchDocumentInner** | `Array<JsonPatchDocumentInner>` | JSON Patch operations to apply to the repository | |
