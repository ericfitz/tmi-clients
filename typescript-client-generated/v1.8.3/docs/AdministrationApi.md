...
|------------- | ------------- | -------------|
| [**addGroupMember**](AdministrationApi.md#addgroupmemberoperation) | **POST** /admin/groups/{group_id}/members | Add member to group |
...
| [**updateAdminGroup**](AdministrationApi.md#updateadmingroupoperation) | **PATCH** /admin/groups/{group_id} | Update group metadata |
| [**updateAdminUser**](AdministrationApi.md#updateadminuseroperation) | **PATCH** /admin/users/{user_id} | Update user metadata |
...
> GroupMember addGroupMember(groupId, addGroupMemberRequest)
...
async function example() {
  ...
  const body = {
    // string | Internal system UUID of the group
...
|------------- | ------------- | ------------- | -------------|
| **groupId** | `string` | Internal system UUID of the group | [Defaults to `undefined`] |
...
> AdminGroup getAdminGroup(groupId)
...
> AdminGroupListResponse listAdminGroups(provider, groupName, usedInAuthorizations, builtIn, limit, offset, sortBy, sortOrder)
...
async function example() {
  ...
  const body = {
    ...
    // boolean | Filter groups by built-in (server-seeded) status (optional)
    builtIn: true,
    // number | Maximum number of results to return (optional)
    limit: 56,
    // number | Number of results to skip (optional)
...
| **builtIn** | `boolean` | Filter groups by built-in (server-seeded) status | [Optional] [Defaults to `undefined`] |
| **limit** | `number` | Maximum number of results to return | [Optional] [Defaults to `50`] |
| **offset** | `number` | Number of results to skip | [Optional] [Defaults to `0`] |
...
> GroupMemberListResponse listGroupMembers(groupId, limit, offset)
...
> ListSystemAuditEntriesResponse listSystemAuditEntries(actorEmail, actorProvider, createdAfter, createdBefore, httpMethod, pathPrefix, fieldPath, limit, cursor, around)

List system audit entries

Cursor-paginated, filterable list of system-level admin-write audit records. Admin role required; read-only (no step-up).  Content negotiation: the default application/json returns a cursor-paginated page. Request Accept: text/csv or application/x-ndjson to stream the entire filtered set as a downloadable export (honors all active filters; ignores cursor/limit/around). An Accept header that matches none of the offered types yields 406 Not Acceptable.
...
> removeGroupMember(groupId, memberUuid, subjectType)
...
> TransferOwnershipResult transferAdminUserOwnership(userId, transferOwnershipRequest)
...
> AdminGroup updateAdminGroup(groupId, updateAdminGroupRequest)
...
> AdminUser updateAdminUser(userId, updateAdminUserRequest)
