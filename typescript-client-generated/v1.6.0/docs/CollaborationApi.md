...
async function example() {
  ...
  const api = new CollaborationApi(config);
  ...
  } satisfies CreateDiagramCollaborationSessionRequest;

  try {
    const data = await api.createDiagramCollaborationSession(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **200** | Collaboration session already exists; the existing session is returned. Clients must NOT evaluate the payload to determine success. |  * X-RateLimit-Limit - Maximum number of requests allowed in the current time window <br>  * X-RateLimit-Remaining - Number of requests remaining in the current time window <br>  * X-RateLimit-Reset - Unix epoch seconds when the rate limit window resets <br>  |
...
async function example() {
  ...
  const body = {
    ...
    // string | Diagram identifier
    diagramId: 38400000-8cf0-11bd-b23e-10b96e4ef00d,
  } satisfies EndDiagramCollaborationSessionRequest;

  try {
    const data = await api.endDiagramCollaborationSession(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
async function example() {
  ...
  try {
    const data = await api.getCurrentUserSessions();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
async function example() {
  ...
  const body = {
    ...
    // string | Diagram identifier
    diagramId: 38400000-8cf0-11bd-b23e-10b96e4ef00d,
  } satisfies GetDiagramCollaborationSessionRequest;

  try {
    const data = await api.getDiagramCollaborationSession(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
