...
class AdminUserIdentitiesResponse(BaseModel):
    """
    ...
    linked: Optional[Annotated[List[LinkedIdentity], Field(max_length=500)]] = Field(default=None, description="Linked sign-in identities; the primary identity is not deletable and does not appear here")
    __properties: ClassVar[List[str]] = ["primary", "linked"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in linked (list)
        _items = []
        if self.linked:
            for _item_linked in self.linked:
                if _item_linked:
                    _items.append(_item_linked.to_dict())
            _dict['linked'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "primary": AdminUserIdentitiesResponsePrimary.from_dict(obj["primary"]) if obj.get("primary") is not None else None,
            "linked": [LinkedIdentity.from_dict(_item) for _item in obj["linked"]] if obj.get("linked") is not None else None
        })
        return _obj
