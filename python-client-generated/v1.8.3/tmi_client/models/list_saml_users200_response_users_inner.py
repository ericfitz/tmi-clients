...
class ListSAMLUsers200ResponseUsersInner(BaseModel):
    """
    ListSAMLUsers200ResponseUsersInner
    """ # noqa: E501
    internal_uuid: UUID = Field(description="TMI-internal identifier for the user.", json_schema_extra={"examples": ["123e4567-e89b-12d3-a456-426614174000"]})
    email: StrictStr = Field(description="User's email address as asserted by the identity provider.", json_schema_extra={"examples": ["alice@example.com"]})
    name: StrictStr = Field(description="User's display name.", json_schema_extra={"examples": ["Alice Smith"]})
    last_login: Optional[datetime] = Field(default=None, description="When the user last authenticated. Omitted if the user has never logged in.", json_schema_extra={"examples": ["2026-01-15T10:30:00Z"]})
    __properties: ClassVar[List[str]] = ["internal_uuid", "email", "name", "last_login"]
