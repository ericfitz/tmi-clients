...
class IdentityLinkStartResponse(BaseModel):
    """
    IdentityLinkStartResponse
    """ # noqa: E501
    link_state: StrictStr = Field(description="Opaque state identifier for the link flow")
    authorization_url: StrictStr = Field(description="URL to redirect the user to for identity provider authorization (prompt=select_account)")
    expires_at: datetime = Field(description="When the link state expires (10 minutes from creation)")
    __properties: ClassVar[List[str]] = ["link_state", "authorization_url", "expires_at"]
