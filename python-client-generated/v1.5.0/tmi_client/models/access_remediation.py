...
class AccessRemediation(BaseModel):
    """
    ...
    """ # noqa: E501
    action: StrictStr
    params: Dict[str, Any] = Field(description="Action-specific parameters. Examples: `service_account_email`, `provider_id`, `user_email` (for `share_with_service_account`); `drive_id`, `item_id`, `app_object_id`, `graph_call`, `graph_body` (for `share_with_application`).")
    __properties: ClassVar[List[str]] = ["action", "params"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['link_account', 'relink_account', 'repick_file', 'share_with_service_account', 'share_with_application', 'repick_after_share', 'retry', 'contact_owner']):
            raise ValueError("must be one of enum values ('link_account', 'relink_account', 'repick_file', 'share_with_service_account', 'share_with_application', 'repick_after_share', 'retry', 'contact_owner')")
        return value
