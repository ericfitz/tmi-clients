...
class ClientConfig(BaseModel):
    ...
    """ # noqa: E501
    features: Optional[ClientConfigFeatures] = None
    operator: Optional[ClientConfigOperator] = None
    limits: Optional[ClientConfigLimits] = None
    ui: Optional[ClientConfigUi] = None
    content_providers: Optional[List[ContentProvider]] = Field(default=None, description="Content providers the server has configured. Order matches server-side registration order.")
    __properties: ClassVar[List[str]] = ["features", "operator", "limits", "ui", "content_providers"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in content_providers (list)
        _items = []
        if self.content_providers:
            for _item_content_providers in self.content_providers:
                if _item_content_providers:
                    _items.append(_item_content_providers.to_dict())
            _dict['content_providers'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "features": ClientConfigFeatures.from_dict(obj["features"]) if obj.get("features") is not None else None,
            "operator": ClientConfigOperator.from_dict(obj["operator"]) if obj.get("operator") is not None else None,
            "limits": ClientConfigLimits.from_dict(obj["limits"]) if obj.get("limits") is not None else None,
            "ui": ClientConfigUi.from_dict(obj["ui"]) if obj.get("ui") is not None else None,
            "content_providers": [ContentProvider.from_dict(_item) for _item in obj["content_providers"]] if obj.get("content_providers") is not None else None
        })
        return _obj
