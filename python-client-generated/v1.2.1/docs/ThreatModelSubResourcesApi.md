...
with tmi_client.ApiClient(configuration) as api_client:
    ...
    threat_type = ['[\"spoofing\",\"tampering\"]'] # List[str] | Filter by threat types (AND logic). Threat must contain ALL specified types. Example: ?threat_type=Tampering&threat_type=Spoofing (optional)
