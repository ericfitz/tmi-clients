...
class Node(Cell):
    ...
    angle: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=0, description="Rotation angle in degrees"
    )
    attrs: Optional[NodeAttrs] = Field(
        default=None, description="Visual styling attributes for the node"
    )
    ports: Optional[PortConfiguration] = Field(
        default=None, description="Port configuration for connections"
    )
    parent: Optional[UUID] = Field(
        default=None,
        description="ID of the parent cell for nested/grouped nodes (UUID)",
    )
    children: Optional[Annotated[List[UUID], Field(max_length=1000)]] = Field(
        default=None,
    ...
    y: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Y coordinate (flat format)"
    )
    width: Optional[
        Union[
            Annotated[float, Field(strict=True, ge=40)],
            Annotated[int, Field(strict=True, ge=40)],
        ]
    ] = Field(default=None, description="Width in pixels (flat format)")
    height: Optional[
        Union[
            Annotated[float, Field(strict=True, ge=30)],
            Annotated[int, Field(strict=True, ge=30)],
        ]
    ] = Field(default=None, description="Height in pixels (flat format)")
    ...
    def shape_validate_enum(cls, value):
        ...
        if value not in set(
            ["actor", "process", "store", "security-boundary", "text-box"]
        ):
            raise ValueError(
                "must be one of enum values ('actor', 'process', 'store', 'security-boundary', 'text-box')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "position": NodeAllOfPosition.from_dict(obj["position"])
                if obj.get("position") is not None
                else None,
                "size": NodeAllOfSize.from_dict(obj["size"])
                if obj.get("size") is not None
                else None,
                "angle": obj.get("angle") if obj.get("angle") is not None else 0,
                "attrs": NodeAttrs.from_dict(obj["attrs"])
                if obj.get("attrs") is not None
                else None,
                "ports": PortConfiguration.from_dict(obj["ports"])
