...
class CreateAddonRequest(BaseModel):
    ...
    description: Optional[
        Annotated[str, Field(min_length=0, strict=True, max_length=2048)]
    ] = Field(default=None, description="Description of what the add-on does")
    icon: Optional[Annotated[str, Field(strict=True, max_length=60)]] = Field(
        default=None,
        description="Icon identifier (Material Symbols or FontAwesome format)",
    )
    objects: Optional[Annotated[List[StrictStr], Field(max_length=100)]] = Field(
        default=None, description="TMI object types this add-on can operate on"
    )
    threat_model_id: Optional[UUID] = Field(
        default=None, description="Optional: Scope add-on to specific threat model"
    )
    parameters: Optional[Annotated[List[AddonParameter], Field(max_length=20)]] = Field(
        default=None,
    ...
    def icon_validate_regular_expression(cls, value):
        ...
        if not re.match(
            r"^(material-symbols:[a-z]([a-z0-9_]*[a-z0-9])?|fa-[a-z]([a-z]*[a-z])?(\-[a-z]+)? fa-([a-z]+)(-[a-z]+)*)$",
            value,
        ):
            raise ValueError(
                r"must validate the regular expression /^(material-symbols:[a-z]([a-z0-9_]*[a-z0-9])?|fa-[a-z]([a-z]*[a-z])?(\-[a-z]+)? fa-([a-z]+)(-[a-z]+)*)$/"
            )
        return value
    ...
    def objects_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(
                ...
                raise ValueError(
                    "each list item must be one of ('threat_model', 'diagram', 'asset', 'threat', 'document', 'note', 'repository', 'metadata', 'survey', 'survey_response')"
                )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "parameters": [
                    AddonParameter.from_dict(_item) for _item in obj["parameters"]
                ]
