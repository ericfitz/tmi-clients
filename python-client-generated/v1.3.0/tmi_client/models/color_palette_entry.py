...
class ColorPaletteEntry(BaseModel):
    ...
    position: Annotated[int, Field(le=8, strict=True, ge=1)] = Field(
        description="Display order position (1-8)"
    )
    color: Annotated[str, Field(min_length=4, strict=True, max_length=7)] = Field(
    ...
    def color_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^#[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$", value):
            raise ValueError(
                r"must validate the regular expression /^#[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/"
            )
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {"position": obj.get("position"), "color": obj.get("color")}
        )
        return _obj
