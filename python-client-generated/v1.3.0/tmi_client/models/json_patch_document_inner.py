...
class JsonPatchDocumentInner(BaseModel):
    """
    JsonPatchDocumentInner
    """  # noqa: E501

    op: Annotated[str, Field(strict=True, max_length=1000)] = Field(
        description="Patch operation type"
    )
    path: Annotated[str, Field(strict=True, max_length=512)] = Field(
    ...
    def op_validate_enum(cls, value):
        ...
        if value not in set(["add", "replace", "remove", "move", "copy", "test"]):
            raise ValueError(
                "must be one of enum values ('add', 'replace', 'remove', 'move', 'copy', 'test')"
            )
        return value
