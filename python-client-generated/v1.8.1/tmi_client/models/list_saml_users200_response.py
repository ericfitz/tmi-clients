...
class ListSAMLUsers200Response(BaseModel):
    """
    Lightweight SAML user list for UI autocomplete.
    """ # noqa: E501
    idp: StrictStr = Field(description="Identity provider the users belong to; always equals the {idp} path parameter.", json_schema_extra={"examples": ["tmi"]})
    users: List[ListSAMLUsers200ResponseUsersInner] = Field(description="Users from this identity provider, most recently logged in first.")
    total: StrictInt = Field(description="Total users matching the filter, ignoring limit/offset.", json_schema_extra={"examples": [41]})
    __properties: ClassVar[List[str]] = ["idp", "users", "total"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "idp": obj.get("idp"),
            "users": [ListSAMLUsers200ResponseUsersInner.from_dict(_item) for _item in obj["users"]] if obj.get("users") is not None else None,
            "total": obj.get("total")
        })
        return _obj
