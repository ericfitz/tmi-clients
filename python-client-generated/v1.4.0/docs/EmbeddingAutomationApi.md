...
with tmi_client.ApiClient(configuration) as api_client:
    ...
    api_instance = tmi_client.EmbeddingAutomationApi(api_client)
    threat_model_id = UUID('38400000-8cf0-11bd-b23e-10b96e4ef00d') # UUID | Threat model identifier
    embedding_ingestion_request = {"index_type":"text","embeddings":[{"entity_type":"asset","entity_id":"123e4567-e89b-12d3-a456-426614174000","chunk_index":0,"chunk_text":"Payment gateway processes credit card transactions","content_hash":"abc123def456","embedding_model":"text-embedding-3-small","embedding_dim":1536,"vector":[0.1,0.2,0.3]}]} # EmbeddingIngestionRequest | Batch of pre-computed embeddings to ingest
