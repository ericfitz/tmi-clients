...
class PendingIdentityLinkResponsePending(BaseModel):
    """
    PendingIdentityLinkResponsePending
    """ # noqa: E501
    provider: StrictStr = Field(description="Identity provider of the second identity")
    provider_user_id: StrictStr = Field(description="User identifier at the provider (first 8 chars + ellipsis)")
    email: Optional[StrictStr] = Field(default=None, description="Cached email from the second identity (display only)")
    name: Optional[StrictStr] = Field(default=None, description="Cached display name from the second identity")
    __properties: ClassVar[List[str]] = ["provider", "provider_user_id", "email", "name"]
