...
class WebhookQuota(BaseModel):
    ...
    max_subscriptions: Annotated[int, Field(strict=True, ge=1)] = Field(
        description="Maximum number of webhook subscriptions"
    )
    max_events_per_minute: Annotated[int, Field(strict=True, ge=1)] = Field(
        description="Maximum webhook events per minute"
    )
    max_subscription_requests_per_minute: Annotated[int, Field(strict=True, ge=1)] = (
        Field(description="Maximum subscription requests per minute")
    )
    max_subscription_requests_per_day: Annotated[int, Field(strict=True, ge=1)] = Field(
        description="Maximum subscription requests per day"
    )
    created_at: Optional[datetime] = Field(
        default=None, description="Timestamp when the quota was created"
    )
    modified_at: Optional[datetime] = Field(
        default=None, description="Timestamp when the quota was last modified"
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "max_subscription_requests_per_minute": obj.get(
                    "max_subscription_requests_per_minute"
                ),
                "max_subscription_requests_per_day": obj.get(
                    "max_subscription_requests_per_day"
                ),
