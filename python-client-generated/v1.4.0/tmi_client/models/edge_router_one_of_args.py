...
class EdgeRouterOneOfArgs(BaseModel):
    ...
    padding: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Padding around obstacles for routing"
    )
    step: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Grid step size for orthogonal routing"
    )
    directions: Optional[
        Annotated[
    ...
    ] = Field(default=None, description="Allowed routing directions")
    ...
    def directions_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(["top", "right", "bottom", "left"]):
                raise ValueError(
                    "each list item must be one of ('top', 'right', 'bottom', 'left')"
                )
        return value
