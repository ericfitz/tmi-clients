...
class PortConfigurationGroupsValue(BaseModel):
    """
    PortConfigurationGroupsValue
    """ # noqa: E501
    position: Optional[StrictStr] = Field(default=None, description="Port position on the node")
    attrs: Optional[Dict[str, Any]] = Field(default=None, description="Visual attributes for port group rendering (e.g., circle styling)")
    __properties: ClassVar[List[str]] = ["position", "attrs"]
    ...
    def position_validate_enum(cls, value):
        ...
        if value not in set(['top', 'right', 'bottom', 'left']):
            raise ValueError("must be one of enum values ('top', 'right', 'bottom', 'left')")
        return value
