...
class EmbeddingIngestionItem(BaseModel):
    """
    EmbeddingIngestionItem
    """ # noqa: E501
    entity_type: StrictStr = Field(description="Entity type (e.g., repository, asset)")
    entity_id: UUID = Field(description="Entity UUID")
    chunk_index: Annotated[int, Field(strict=True, ge=0)] = Field(description="Sequential chunk number")
    chunk_text: StrictStr = Field(description="Original text of the chunk")
    content_hash: StrictStr = Field(description="SHA256 hash of the original content")
    embedding_model: StrictStr = Field(description="Model used to generate the embedding")
    embedding_dim: Annotated[int, Field(strict=True, ge=1)] = Field(description="Embedding vector dimension")
    vector: Annotated[List[Union[StrictFloat, StrictInt]], Field(max_length=4096)] = Field(description="Embedding vector")
    __properties: ClassVar[List[str]] = ["entity_type", "entity_id", "chunk_index", "chunk_text", "content_hash", "embedding_model", "embedding_dim", "vector"]
