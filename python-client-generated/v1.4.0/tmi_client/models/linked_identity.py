...
class LinkedIdentity(BaseModel):
    ...
    LinkedIdentity
    """ # noqa: E501
    id: UUID = Field(description="Linked identity unique identifier")
    provider: StrictStr = Field(description="Identity provider ID")
    provider_user_id: StrictStr = Field(description="User identifier at the provider (truncated for display)")
    email: Optional[StrictStr] = Field(default=None, description="Cached email address from provider (display only)")
    name: Optional[StrictStr] = Field(default=None, description="Cached display name from provider")
    linked_at: datetime = Field(description="When this identity was linked")
    last_used_at: Optional[datetime] = Field(default=None, description="When this identity was last used to sign in")
    __properties: ClassVar[List[str]] = ["id", "provider", "provider_user_id", "email", "name", "linked_at", "last_used_at"]
