...
class SystemSetting(BaseModel):
    ...
    """ # noqa: E501
    key: Annotated[str, Field(strict=True, max_length=256)] = Field(description="Unique setting identifier using dot notation (e.g., rate_limit.requests_per_minute)")
    value: Annotated[str, Field(strict=True, max_length=4000)] = Field(description="Setting value as a string (interpreted based on type)")
    type: StrictStr = Field(description="Data type of the setting value")
    description: Optional[Annotated[str, Field(strict=True, max_length=2048)]] = Field(default=None, description="Human-readable description of the setting")
    modified_at: Optional[datetime] = Field(default=None, description="When the setting was last modified")
    modified_by: Optional[UUID] = Field(default=None, description="UUID of the user who last modified the setting")
    source: Optional[StrictStr] = Field(default=None, description="Where the effective value of this setting comes from. Server-managed, not writable.")
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['string', 'int', 'bool', 'json']):
            raise ValueError("must be one of enum values ('string', 'int', 'bool', 'json')")
        return value
    ...
    def source_validate_enum(cls, value):
        ...
        if value not in set(['database', 'config', 'environment', 'vault']):
            raise ValueError("must be one of enum values ('database', 'config', 'environment', 'vault')")
        return value
