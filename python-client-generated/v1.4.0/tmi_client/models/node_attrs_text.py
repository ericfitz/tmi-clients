...
class NodeAttrsText(BaseModel):
    ...
    text: Optional[Annotated[str, Field(strict=True, max_length=256)]] = Field(
        default=None, description="Label text content"
    )
    font_size: Optional[
        Union[
    ...
    ] = Field(default=None, description="Font size in pixels", alias="fontSize")
    fill: Optional[Annotated[str, Field(strict=True, max_length=32)]] = Field(
        default=None, description="Text color"
    )
    font_family: Optional[Annotated[str, Field(strict=True, max_length=64)]] = Field(
        default=None, description="Font family", alias="fontFamily"
    )
    ...
    ref_dx: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Horizontal offset from refX", alias="refDx"
    )
    ref_dy: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Vertical offset from refY", alias="refDy"
    )
    text_anchor: Optional[StrictStr] = Field(
        default=None,
        description="Horizontal text alignment anchor point",
        alias="textAnchor",
    )
    text_vertical_anchor: Optional[StrictStr] = Field(
        default=None,
        description="Vertical text alignment anchor point",
        alias="textVerticalAnchor",
    )
    ...
    def fill_validate_regular_expression(cls, value):
        ...
        value = value.isoformat() if hasattr(value, "isoformat") else str(value)
        if not re.match(
            r"^(#[0-9a-fA-F]*|#[0-9a-fA-F]*|rgb\( *[0-9]+ *, *[0-9]+ *, *[0-9]+ *\)|[a-z]+)$",
            value,
        ):
            raise ValueError(
                r"must validate the regular expression /^(#[0-9a-fA-F]*|#[0-9a-fA-F]*|rgb\( *[0-9]+ *, *[0-9]+ *, *[0-9]+ *\)|[a-z]+)$/"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "refX": NodeAttrsTextRefX.from_dict(obj["refX"])
                if obj.get("refX") is not None
                else None,
                "refY": NodeAttrsTextRefY.from_dict(obj["refY"])
                if obj.get("refY") is not None
                else None,
                "refDx": obj.get("refDx"),
                "refDy": obj.get("refDy"),
