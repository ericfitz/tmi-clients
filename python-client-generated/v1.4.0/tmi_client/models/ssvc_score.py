...
class SSVCScore(BaseModel):
    """
    SSVC (Stakeholder-Specific Vulnerability Categorization) assessment result
    """ # noqa: E501
    vector: Annotated[str, Field(min_length=1, strict=True, max_length=256)] = Field(description="SSVC vector string following CERT/CC convention (e.g., SSVCv2/E:A/U:S/T:T/P:S/2026-04-08/)")
    decision: StrictStr = Field(description="SSVC decision outcome")
    methodology: Annotated[str, Field(min_length=1, strict=True, max_length=64)] = Field(description="The SSVC stakeholder perspective used for assessment")
    __properties: ClassVar[List[str]] = ["vector", "decision", "methodology"]
    ...
    def decision_validate_enum(cls, value):
        """Validates the enum"""
        value = value.isoformat() if hasattr(value, 'isoformat') else str(value)
        if value not in set(['Defer', 'Scheduled', 'Out-of-Cycle', 'Immediate']):
            raise ValueError("must be one of enum values ('Defer', 'Scheduled', 'Out-of-Cycle', 'Immediate')")
        return value

    @field_validator('methodology')
    def methodology_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        value = value.isoformat() if hasattr(value, 'isoformat') else str(value)
        if not re.match(r"^[A-Za-z0-9 _-]+$", value):
            raise ValueError(r"must validate the regular expression /^[A-Za-z0-9 _-]+$/")
