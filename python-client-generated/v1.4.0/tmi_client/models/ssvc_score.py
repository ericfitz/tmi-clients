...
class SSVCScore(BaseModel):
    ...
    vector: Annotated[str, Field(min_length=1, strict=True, max_length=256)] = Field(
    ...
    methodology: Annotated[str, Field(min_length=1, strict=True, max_length=64)] = (
        Field(description="The SSVC stakeholder perspective used for assessment")
    )
    ...
    def decision_validate_enum(cls, value):
        ...
        if value not in set(["Defer", "Scheduled", "Out-of-Cycle", "Immediate"]):
            raise ValueError(
                "must be one of enum values ('Defer', 'Scheduled', 'Out-of-Cycle', 'Immediate')"
            )
        return value
