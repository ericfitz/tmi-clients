...
class EmbeddingIngestionRequest(BaseModel):
    ...
    embeddings: Annotated[
        List[EmbeddingIngestionItem], Field(min_length=1, max_length=1000)
    ] = Field(description="Batch of pre-computed embeddings")
    __properties: ClassVar[List[str]] = ["index_type", "embeddings"]

    @field_validator("index_type")
    def index_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["text", "code"]):
            raise ValueError("must be one of enum values ('text', 'code')")
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "embeddings": [
                    EmbeddingIngestionItem.from_dict(_item)
                    for _item in obj["embeddings"]
                ]
