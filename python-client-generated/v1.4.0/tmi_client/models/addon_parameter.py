...
class AddonParameter(BaseModel):
    ...
    description: Optional[Annotated[str, Field(strict=True, max_length=512)]] = Field(
        default=None, description="Human-readable description for UI display"
    )
    ...
    enum_values: Optional[
        Annotated[
            List[Annotated[str, Field(strict=True, max_length=256)]],
            Field(max_length=50),
        ]
    ] = Field(
        default=None, description="Allowed values (applicable when type is 'enum')"
    )
    default_value: Optional[Annotated[str, Field(strict=True, max_length=256)]] = Field(
        default=None, description="Default value if not provided by user"
    )
    metadata_key: Optional[Annotated[str, Field(strict=True, max_length=256)]] = Field(
        default=None,
        description="Metadata key name to auto-populate from TMI object (applicable when type is 'metadata_key')",
    )
    number_min: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None,
        description="Minimum allowed value (applicable when type is 'number')",
    )
    number_max: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None,
        description="Maximum allowed value (applicable when type is 'number')",
    )
    string_max_length: Optional[Annotated[int, Field(le=10000, strict=True, ge=1)]] = (
        Field(
            default=None,
            description="Maximum string length (applicable when type is 'string')",
        )
    )
    string_validation_regex: Optional[
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(["enum", "boolean", "string", "number", "metadata_key"]):
            raise ValueError(
                "must be one of enum values ('enum', 'boolean', 'string', 'number', 'metadata_key')"
            )
        return value
