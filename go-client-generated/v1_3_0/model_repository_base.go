/*
TMI (Threat Modeling Improved) API

A RESTful API for collaborative threat modeling with full X6 graph library compatibility. This API provides schemas that align with AntV X6 cell object models for seamless integration with modern diagramming libraries. Supports OAuth 2.0 authentication with client callback integration for seamless single-page application authentication flows.  ## API Design v1.1.0  ### Authorization Model TMI uses hierarchical authorization: access control is defined at the ThreatModel level via the authorization field (readers, writers, owners). All child resources (Assets, Diagrams, Documents, Notes, Repositories, Threats) inherit permissions from their parent ThreatModel. This simplifies permission management and ensures consistent access control.  ### Bulk Operations Notes and Diagrams do not support bulk operations due to their unique creation workflows and lack of valid bulk use cases. All other resources (Threats, Assets, Documents, Repositories) support full bulk operations: POST (create), PUT (upsert), PATCH (partial update), DELETE (batch delete).  All resources support bulk metadata operations regardless of resource-level bulk support.  ### List Response Strategy - ThreatModels return summary information (TMListItem) because they contain many child objects that can be large. - Diagrams return summary information (DiagramListItem) because diagram data (cells, images) can be large. - Notes return summary information (NoteListItem) because the content field can be large. - Threats, Assets, Documents, Repositories return full schemas as they are relatively small and static.  ### PATCH Support All resources support PATCH for partial updates using JSON Patch (RFC 6902). This is particularly useful for: - Assets: Array field updates (affected_assets, trust_boundaries) ensuring no duplicates - Notes: Updating name/description without changing content field - All resources: Efficient updates without full object replacement 

API version: 1.3.0
Contact: eric@tmi.dev
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tmiclient

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the RepositoryBase type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &RepositoryBase{}

// RepositoryBase Base fields for Repository (user-writable only)
type RepositoryBase struct {
	// Name for the source code reference
	Name NullableString `json:"name,omitempty" validate:"regexp=^[^<>\\"'&]*$"`
	// Description of the referenced source code
	Description NullableString `json:"description,omitempty" validate:"regexp=^[^<>\\\\x00-\\\\x08\\\\x0B\\\\x0C\\\\x0E-\\\\x1F]*$"`
	// Source code repository type
	Type NullableString `json:"type,omitempty"`
	Parameters *RepositoryBaseParameters `json:"parameters,omitempty"`
	// URL to retrieve the referenced source code
	Uri string `json:"uri" validate:"regexp=^[a-zA-Z][a-zA-Z0-9+.-]*:\\/\\/[^\\\\s]*$"`
	// Whether this item should be included in generated reports
	IncludeInReport *bool `json:"include_in_report,omitempty"`
	// Whether the Timmy AI assistant is enabled for this entity
	TimmyEnabled *bool `json:"timmy_enabled,omitempty"`
}

type _RepositoryBase RepositoryBase

// NewRepositoryBase instantiates a new RepositoryBase object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRepositoryBase(uri string) *RepositoryBase {
	this := RepositoryBase{}
	this.Uri = uri
	var includeInReport bool = true
	this.IncludeInReport = &includeInReport
	var timmyEnabled bool = true
	this.TimmyEnabled = &timmyEnabled
	return &this
}

// NewRepositoryBaseWithDefaults instantiates a new RepositoryBase object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRepositoryBaseWithDefaults() *RepositoryBase {
	this := RepositoryBase{}
	var includeInReport bool = true
	this.IncludeInReport = &includeInReport
	var timmyEnabled bool = true
	this.TimmyEnabled = &timmyEnabled
	return &this
}

// GetName returns the Name field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *RepositoryBase) GetName() string {
	if o == nil || IsNil(o.Name.Get()) {
		var ret string
		return ret
	}
	return *o.Name.Get()
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *RepositoryBase) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Name.Get(), o.Name.IsSet()
}

// HasName returns a boolean if a field has been set.
func (o *RepositoryBase) HasName() bool {
	if o != nil && o.Name.IsSet() {
		return true
	}

	return false
}

// SetName gets a reference to the given NullableString and assigns it to the Name field.
func (o *RepositoryBase) SetName(v string) {
	o.Name.Set(&v)
}
// SetNameNil sets the value for Name to be an explicit nil
func (o *RepositoryBase) SetNameNil() {
	o.Name.Set(nil)
}

// UnsetName ensures that no value is present for Name, not even an explicit nil
func (o *RepositoryBase) UnsetName() {
	o.Name.Unset()
}

// GetDescription returns the Description field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *RepositoryBase) GetDescription() string {
	if o == nil || IsNil(o.Description.Get()) {
		var ret string
		return ret
	}
	return *o.Description.Get()
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *RepositoryBase) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Description.Get(), o.Description.IsSet()
}

// HasDescription returns a boolean if a field has been set.
func (o *RepositoryBase) HasDescription() bool {
	if o != nil && o.Description.IsSet() {
		return true
	}

	return false
}

// SetDescription gets a reference to the given NullableString and assigns it to the Description field.
func (o *RepositoryBase) SetDescription(v string) {
	o.Description.Set(&v)
}
// SetDescriptionNil sets the value for Description to be an explicit nil
func (o *RepositoryBase) SetDescriptionNil() {
	o.Description.Set(nil)
}

// UnsetDescription ensures that no value is present for Description, not even an explicit nil
func (o *RepositoryBase) UnsetDescription() {
	o.Description.Unset()
}

// GetType returns the Type field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *RepositoryBase) GetType() string {
	if o == nil || IsNil(o.Type.Get()) {
		var ret string
		return ret
	}
	return *o.Type.Get()
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *RepositoryBase) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Type.Get(), o.Type.IsSet()
}

// HasType returns a boolean if a field has been set.
func (o *RepositoryBase) HasType() bool {
	if o != nil && o.Type.IsSet() {
		return true
	}

	return false
}

// SetType gets a reference to the given NullableString and assigns it to the Type field.
func (o *RepositoryBase) SetType(v string) {
	o.Type.Set(&v)
}
// SetTypeNil sets the value for Type to be an explicit nil
func (o *RepositoryBase) SetTypeNil() {
	o.Type.Set(nil)
}

// UnsetType ensures that no value is present for Type, not even an explicit nil
func (o *RepositoryBase) UnsetType() {
	o.Type.Unset()
}

// GetParameters returns the Parameters field value if set, zero value otherwise.
func (o *RepositoryBase) GetParameters() RepositoryBaseParameters {
	if o == nil || IsNil(o.Parameters) {
		var ret RepositoryBaseParameters
		return ret
	}
	return *o.Parameters
}

// GetParametersOk returns a tuple with the Parameters field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RepositoryBase) GetParametersOk() (*RepositoryBaseParameters, bool) {
	if o == nil || IsNil(o.Parameters) {
		return nil, false
	}
	return o.Parameters, true
}

// HasParameters returns a boolean if a field has been set.
func (o *RepositoryBase) HasParameters() bool {
	if o != nil && !IsNil(o.Parameters) {
		return true
	}

	return false
}

// SetParameters gets a reference to the given RepositoryBaseParameters and assigns it to the Parameters field.
func (o *RepositoryBase) SetParameters(v RepositoryBaseParameters) {
	o.Parameters = &v
}

// GetUri returns the Uri field value
func (o *RepositoryBase) GetUri() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Uri
}

// GetUriOk returns a tuple with the Uri field value
// and a boolean to check if the value has been set.
func (o *RepositoryBase) GetUriOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Uri, true
}

// SetUri sets field value
func (o *RepositoryBase) SetUri(v string) {
	o.Uri = v
}

// GetIncludeInReport returns the IncludeInReport field value if set, zero value otherwise.
func (o *RepositoryBase) GetIncludeInReport() bool {
	if o == nil || IsNil(o.IncludeInReport) {
		var ret bool
		return ret
	}
	return *o.IncludeInReport
}

// GetIncludeInReportOk returns a tuple with the IncludeInReport field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RepositoryBase) GetIncludeInReportOk() (*bool, bool) {
	if o == nil || IsNil(o.IncludeInReport) {
		return nil, false
	}
	return o.IncludeInReport, true
}

// HasIncludeInReport returns a boolean if a field has been set.
func (o *RepositoryBase) HasIncludeInReport() bool {
	if o != nil && !IsNil(o.IncludeInReport) {
		return true
	}

	return false
}

// SetIncludeInReport gets a reference to the given bool and assigns it to the IncludeInReport field.
func (o *RepositoryBase) SetIncludeInReport(v bool) {
	o.IncludeInReport = &v
}

// GetTimmyEnabled returns the TimmyEnabled field value if set, zero value otherwise.
func (o *RepositoryBase) GetTimmyEnabled() bool {
	if o == nil || IsNil(o.TimmyEnabled) {
		var ret bool
		return ret
	}
	return *o.TimmyEnabled
}

// GetTimmyEnabledOk returns a tuple with the TimmyEnabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RepositoryBase) GetTimmyEnabledOk() (*bool, bool) {
	if o == nil || IsNil(o.TimmyEnabled) {
		return nil, false
	}
	return o.TimmyEnabled, true
}

// HasTimmyEnabled returns a boolean if a field has been set.
func (o *RepositoryBase) HasTimmyEnabled() bool {
	if o != nil && !IsNil(o.TimmyEnabled) {
		return true
	}

	return false
}

// SetTimmyEnabled gets a reference to the given bool and assigns it to the TimmyEnabled field.
func (o *RepositoryBase) SetTimmyEnabled(v bool) {
	o.TimmyEnabled = &v
}

func (o RepositoryBase) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o RepositoryBase) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Name.IsSet() {
		toSerialize["name"] = o.Name.Get()
	}
	if o.Description.IsSet() {
		toSerialize["description"] = o.Description.Get()
	}
	if o.Type.IsSet() {
		toSerialize["type"] = o.Type.Get()
	}
	if !IsNil(o.Parameters) {
		toSerialize["parameters"] = o.Parameters
	}
	toSerialize["uri"] = o.Uri
	if !IsNil(o.IncludeInReport) {
		toSerialize["include_in_report"] = o.IncludeInReport
	}
	if !IsNil(o.TimmyEnabled) {
		toSerialize["timmy_enabled"] = o.TimmyEnabled
	}
	return toSerialize, nil
}

func (o *RepositoryBase) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"uri",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varRepositoryBase := _RepositoryBase{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varRepositoryBase)

	if err != nil {
		return err
	}

	*o = RepositoryBase(varRepositoryBase)

	return err
}

type NullableRepositoryBase struct {
	value *RepositoryBase
	isSet bool
}

func (v NullableRepositoryBase) Get() *RepositoryBase {
	return v.value
}

func (v *NullableRepositoryBase) Set(val *RepositoryBase) {
	v.value = val
	v.isSet = true
}

func (v NullableRepositoryBase) IsSet() bool {
	return v.isSet
}

func (v *NullableRepositoryBase) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRepositoryBase(val *RepositoryBase) *NullableRepositoryBase {
	return &NullableRepositoryBase{value: val, isSet: true}
}

func (v NullableRepositoryBase) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRepositoryBase) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


