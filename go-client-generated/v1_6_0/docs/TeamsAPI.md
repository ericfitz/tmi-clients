...
> Team CreateTeam(ctx).TeamInput(teamInput).Execute()
...
func main() {
	teamInput := *openapiclient.NewTeamInput("Platform Engineering") // TeamInput | Team creation data

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.TeamsAPI.CreateTeam(context.Background()).TeamInput(teamInput).Execute()
	if err != nil {
...
> Team UpdateTeam(ctx, teamId).TeamInput(teamInput).IfMatch(ifMatch).Execute()
...
func main() {
	teamId := "38400000-8cf0-11bd-b23e-10b96e4ef00d" // string | Team UUID
	teamInput := *openapiclient.NewTeamInput("Platform Engineering") // TeamInput | Complete team data for replacement
	ifMatch := "ifMatch_example" // string | Optimistic-locking precondition. Pass the integer version returned by the previous read (or as the body 'version' field on the previous write). On version mismatch the server returns 409 Conflict. In a future release this header will be required and missing values will return 428 Precondition Required. Accepts an integer version (bare or ETag-quoted) or \"*\" for an unconditional write (RFC 7232). (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.TeamsAPI.UpdateTeam(context.Background(), teamId).TeamInput(teamInput).IfMatch(ifMatch).Execute()
	if err != nil {
