...
> DeleteAdminSurvey(ctx, surveyId).Force(force).Execute()
...
func main() {
	surveyId := "38400000-8cf0-11bd-b23e-10b96e4ef00d" // string | Unique identifier of the survey
	force := true // bool | When true, delete the survey even if it has responses, cascading deletion to all of its responses and their sub-resources (triage notes, access grants, metadata, answers). Intended as an admin cleanup affordance; without it a survey that has responses cannot be deleted (409). (optional) (default to false)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	r, err := apiClient.SurveyAdministrationAPI.DeleteAdminSurvey(context.Background(), surveyId).Force(force).Execute()
	if err != nil {
 ...
 **force** | **bool** | When true, delete the survey even if it has responses, cascading deletion to all of its responses and their sub-resources (triage notes, access grants, metadata, answers). Intended as an admin cleanup affordance; without it a survey that has responses cannot be deleted (409). | [default to false]
