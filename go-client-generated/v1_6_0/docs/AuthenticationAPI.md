...
[**ListMyGroupMembers**](AuthenticationAPI.md#ListMyGroupMembers) | **Get** /me/groups/{group_id}/members | List members of my group
...
> GroupMemberListResponse ListMyGroupMembers(ctx, groupId).Limit(limit).Offset(offset).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.AuthenticationAPI.ListMyGroupMembers(context.Background(), groupId).Limit(limit).Offset(offset).Execute()
	if err != nil {
...
func main() {
	tokenRefreshRequest := *openapiclient.NewTokenRefreshRequest() // TokenRefreshRequest | OAuth 2.0 token refresh request parameters (optional)
